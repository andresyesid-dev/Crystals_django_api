...
def insert_specific_reporting_order(request):
    ...
    try:
        ...
        count = SpecificReportingOrder.objects.count()
        if count > 0:
            return JsonResponse({
                'ok': True,
                'already_exists': True,
        ...
        # Bulk create
        records = [SpecificReportingOrder(**item) for item in default_data]
...
def insert_general_reporting_order(request):
    ...
    try:
        # Check if table already has data
        count = GeneralReportingOrder.objects.count()
        if count > 0:
            return JsonResponse({
                'ok': True,
                'already_exists': True,
        ...
        # Bulk create
        records = [GeneralReportingOrder(**item) for item in default_data]
...
def insert_laboratory_reporting_order(request):
    ...
    try:
        # Check if table already has data
        count = LaboratoryReportingOrder.objects.count()
        if count > 0:
            return JsonResponse({
                'ok': True,
                'already_exists': True,
        ...
        # Bulk create
        records = [LaboratoryReportingOrder(**item) for item in default_data]
