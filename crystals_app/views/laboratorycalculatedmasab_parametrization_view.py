...
_CACHE_GROUP = 'lab_calc_masa_b_param'
...
def get_laboratory_calculated_masa_b_parametrization(request: HttpRequest):
    try:
        data = []
        for o in LaboratoryCalculatedMasaBParametrization.objects.filter(factory_id=request.META.get('HTTP_X_FACTORY_ID', 1)):
        ...
        return JsonResponse({"message": "❌ Error al obtener parametrización Masa B calculada", "error": str(e)}, status=500)
...
def update_laboratory_calculated_masa_b_parametrization(request: HttpRequest):
    try:
        ...
        return JsonResponse({"message": "✅ Parametrización Masa B calculada actualizada exitosamente", "ok": True})
    except Exception as e:
        return JsonResponse({"message": "❌ Error al actualizar parametrización Masa B calculada", "error": str(e)}, status=500)
