...
def add_historic_report(request: HttpRequest):
    ...
    if not calibration_name or not analysis_datetime or not metrics:
        return JsonResponse({"error": "datetime, calibration and metrics required"}, status=400)
...
def get_historic_reports(request: HttpRequest):
    ...
    if not start or not end:
        return JsonResponse({"error": "start and end required (YYYY-mm-dd HH:MM:SS)"}, status=400)
...
def get_historic_reports_for_process(request: HttpRequest):
    ...
    if not all([start, end, process]):
        return JsonResponse({"error": "start, end, process required"}, status=400)
    qs = HistoricReport.objects.filter(datetime__gte=start, datetime__lte=end, calibration=process)
    return JsonResponse({"results": [model_to_dict(o) for o in qs.order_by("datetime")]})
...
def get_last_report(request: HttpRequest):
    ...
    obj = HistoricReport.objects.filter(datetime__gte=start, datetime__lte=end).order_by("-datetime").first()
...
def delete_management_record(request: HttpRequest):
    ...
    if not hr_id:
        return JsonResponse({"error": "hr_id required"}, status=400)
